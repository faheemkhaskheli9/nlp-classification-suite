"""Verification helpers for Phase 3 (archive the 2 source repos).
...
The unit tests exercise this logic against small in-memory/tmp_path
fixtures (so they run in any checkout, including CI where the sibling
`portfolio-archived-repos` directory isn't present). Confirming it against
the *real* archived repos and the real `PORTFOLIO_INDEX.md` is a local,
one-off check — see `scripts/verify_archival.py`.
"""
